...
- **Eager embed bridge** (`neml2/eager.py`, `neml2/_eager_boundary.py`): the C++ `neml2::eager::Model` marshals raw `at::Tensor`s across the embedded-Python boundary via torch's pybind casters. Raw tensors appear only in `forward` / `jvp` / `jacobian`'s argument / return dicts; inputs are wrapped to typed immediately on entry and outputs are unwrapped only at the return (in the `_eager_boundary` `unwrap_outputs` / `assemble_jvp_outputs` / `assemble_jacobian` helpers).
