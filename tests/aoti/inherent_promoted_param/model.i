# inherent_promoted_param: a parameter promoted to a runtime input via HIT (the same
# mode-3/4 path the calibration workflow uses). The exporter must see the
# pre-existing _promoted_params entry on the leaf and emit a graph that takes the
...
[Models]
  [model]
    ...
    # `value` to its input_spec and registers an PromotedParam entry for it.
    value = 'value'
  []
[]
