...
[Tensors]
  [edges]
    # Coarsened for regression-test runtime: 201 -> 51 (50 bins). The full-
  ...
  [unscaled_ic]
    type = Python
    expr = 'Scalar(1e-20 * torch.ones(50, dtype=torch.float64), sub_batch_ndim=1)'
  []
  ...
  [time]
    type = Python
    expr = 'Scalar(torch.logspace(-4, 1, 100, dtype=torch.float64))'
  []
