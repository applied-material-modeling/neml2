# neml2
# Rate-independent J2 plasticity with combined linear isotropic + linear
# kinematic hardening, solved by radial return. The trial state precomputes the
# invariant flow directions (NM, Nk, NX) from a frozen (n-1) state, so the only
# unknown solved in the implicit return map is the scalar flow rate via the
# Fischer-Burmeister consistency condition. Strain history is a linear ramp
# from 0 to max_strain = (0.1, -0.05, -0.05, 0, 0, 0) over 100 steps, batched
# over 20 end-time values logspaced from 1e-1 to 1e5.
[Tensors]
  [end_time]
    type = Python
