# Translated from tests/unit/models/chemical_reactions/ContractingGeometry.i.
[Drivers]
  [unit]
    type = ModelUnitTest
    model = 'model'
    input_Scalar_names = 'alpha'
    input_Scalar_values = 'alpha'
    output_Scalar_names = 'out'
    output_Scalar_values = 'out'
  []
[]

[Tensors]
  [alpha]
    type = Python
    expr = "Scalar(torch.tensor([0.03, 0.75, 0.9], dtype=torch.float64))"
  []
  [out]
    type = Python
    expr = "Scalar(torch.tensor([0.6437545, 0.01546796, 1.244795587e-3], dtype=torch.float64))"
  []
[]

[Models]
  [model]
    type = ContractingGeometry
    coef = 0.7
    order = 2.75
    conversion_degree = 'alpha'
    reaction_rate = 'out'
  []
[]
