# Population balance: grinding on 20 uniform density bins.
# Larger particles fragment faster (gamma ~ bin index, sink at bin 0); each
# parent splits its mass equally over all smaller bins (p[k,j] = v_j / (j v_k)
# for k < j), which conserves mass at uniform density. A coarse Gaussian feed
# grinds toward the fines over time. Runnable with `neml2-run grinding.i driver`.
[Tensors]
  [cell_volume_val]
    type = Python
